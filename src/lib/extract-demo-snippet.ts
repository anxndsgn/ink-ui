 ...
 */
export function extractDemoSnippet(source: string, functionName: string): string {
  ...
  for (const imp of imports) {
    const usedDefault = imp.defaultImport && usedIdentifiers.has(imp.defaultImport);
    const usedNamed = imp.namedImports.filter((name) => usedIdentifiers.has(name));
    const usedNamespace = imp.namespaceImport && usedIdentifiers.has(imp.namespaceImport);
    ...
    usedImports.push(`import ${parts.join(", ")} from "${imp.moduleSpecifier}";`);
  }
