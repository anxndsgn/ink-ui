    ...
    hooks: {
      ...
      "astro:build:done": async ({ dir, logger }) => {
        ...
        for (const htmlFile of htmlFiles) {
          try {
            if (htmlFile.includes(changelogDir) && !htmlFile.endsWith(changelogAllPage)) {
              skipped++;
              continue;
            }
            ...
            const relativeToOut = path.relative(outDir, htmlFile);
            const mdRelative =
              relativeToOut === "index.html"
                ? "index.md"
                : relativeToOut.replace(/\/index\.html$/, ".md");
            const mdFile = htmlFile.endsWith(changelogAllPage)
              ? join(outDir, "changelog.md")
              : join(outDir, mdRelative);
            await writeFile(mdFile, markdown, "utf-8");
            generated++;
          } catch (error) {
            logger.warn(`Failed to generate markdown for ${htmlFile}: ${error}`);
            skipped++;
          }
        }
