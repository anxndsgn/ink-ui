...
export function createTurndownService(): TurndownService {
  ...
  turndown.remove((node) => {
    ...
    if ("getAttribute" in node && (node as Element).getAttribute("data-copy-ignore") !== null) {
      return true;
    }
...
export function htmlToMarkdown(html: string): string {
  ...
  const descMatch = pageHeader.match(/<p[^>]*class="[^"]*text-lg[^"]*"[^>]*>([\s\S]*?)<\/p>/i);
  ...
  const githubMatch = pageHeader.match(/href="(https:\/\/github\.com\/[^"]*)"/i);
  if (githubMatch) {
